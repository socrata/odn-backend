...
module.exports = (socket, request) => {
    socket.on('message', messageString => {
        ...
        const token = message.app_token;

        Promise.all([
            getSession(message),
            getZoomLevel(message),
            getBounds(message)
            ...
            getEntitiesInBounds(entityType, bounds, token)
                ...
                .then(ids => {
                    ...
                    idGroups.forEach(group => {
                        Promise.all([
                            getGeodata(entityType, zoomLevel, group, token),
                            getData(dataset, constraints, group, token)
                        ]).then(([geojson, values]) => {
                            geojson = joinGeoWithData(geojson, values);
                            socket.send(JSON.stringify({geojson, message}));
                        }).catch(error => {
                            console.log(error);
                        });
                    });
...
function getZoomLevel(message) {
    let zoomLevel = message.zoom_level;
